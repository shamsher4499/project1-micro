...
def sendOTPUserProfile(user):
    email = EmailTemplate.objects.get(name='Verify OTP for show information')
    otp = random.randint(1000, 9999)
    subject = email.name
    data = email.editor
    message = data.replace('name', user.user.name).replace('otp', str(otp))
    EmailThread(subject, message, [user.user.email]).start()
    user.otp = otp
    user.save()